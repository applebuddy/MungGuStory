...
CatBossMonster::CatBossMonster(int monsterNumber)
{
...
CatBossMonster::~CatBossMonster()
{
    
    
}

void CatBossMonster::initData()
{
...
void CatBossMonster::loadMonsterSpriteData()
{
    ...
    moveVelocity = 40*winSize.width/1200;
    dieMoney = 100;
    unitSize = 2;
    //기본 이동 액션
    cache->addSpriteFramesWithFile("Cat.plist");
    for (int i = 0; i<4; i++) {
        char *pString[20] = {};
        sprintf((char *)pString, "Cat_%d.png", i);
        moveFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    cache->addSpriteFramesWithFile("Cat_attack.plist");
    for (int i = 0; i<4; i++){
        char *pString[20] = {};
        sprintf((char *)pString, "Cat_attack_%d.png", i);
        attackFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    cache->addSpriteFramesWithFile("Cat_die.plist");
    dieFrames.pushBack(cache->getSpriteFrameByName("Cat_die.png"));
    ...
    animation.attackFrameValue = 1.0f / 8.0f; //공격 이미지 전환 속도
    ...
    monsterStopImage[0] = cache->getSpriteFrameByName("Cat_0.png");
    //monsterDeadImage[0] = cache->getSpriteFrameByName("Cat_die.png");
    scaleRate = 1.8*winSize.width/1200;
...
void CatBossMonster::monsterAction(int actionNumber)
{
    switch (actionNumber) {
        case ACTION_MOVE:
            moveAction();//움직이는 액션
...
void CatBossMonster::setHpBar(){
    ...
    catEnergyTimer->setAnchorPoint(Point(0.77,0));
    catEnergyTimer->setScale(0.55);
    catEnergyTimer->setPosition(Point(mungRect.getMaxX()*0.9,mungRect.getMaxY()*2));
    this->addChild(catEnergyTimer,10);
}

void CatBossMonster::deadAction(){//구현중..
    ...
    if (isFinishHittedAction == true){
        ...
        auto hittedMove2 = JumpBy::create(0.3*frameRate, Point(16*winSize.width/1200 * 1.1, 0), 20, 1);
        auto hittedMove3 = JumpBy::create(0.2*frameRate, Point(10*winSize.width/1200 * 1.1, 0), 10, 1);
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(CatBossMonster::deadAndDisappear)), NULL);
...
void CatBossMonster::angryAction(){
    auto act0 = ScaleBy::create(0, 1.2); //구현중
    auto act = Sequence::create(CallFunc::create(this, callfunc_selector(CatBossMonster::angryEffect)), act0, NULL);
    act->setTag(ACTION_ANGRY);
    runAction(act);
    
}

void CatBossMonster::angryEffect(){//구현중
    //this->setScale(2.0);
    this->hp = hp*1.3;
    this->now_hp = hp;
...
void CatBossMonster::deadAndDisappear(){
...
void CatBossMonster::hittedAction(){
    ...
    if ((isFinishHittedAction == true)&&(now_hp!=0)){
        ...
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(CatBossMonster::hittedActionFinished)), NULL);
...
void CatBossMonster::stopAction() {//게임신에서 멈춤 지시가 오면
...
void CatBossMonster::moveAction() {
    if ((isFinishMoveAction == true) && (isFinishHittedAction == true)){//움직임 액션이 끝났으면 움직임 액션 진행
        
        isFinishMoveAction = false;
        FiniteTimeAction* moveAction = Sequence::create(animation.animate_move, CallFunc::create(this, callfunc_selector(CatBossMonster::moveActionFinished)), NULL);//
...
void CatBossMonster::attackAction(){
    if ((isFinishAttackAction == true) && (isFinishHittedAction == true)) {
        ...
        auto act1 = Sequence::create(act00, CallFunc::create(this, callfunc_selector(CatBossMonster::isAttackEffect)), NULL);
        FiniteTimeAction* attackAction = Sequence::create(animation.animate_attack, CallFunc::create(this, callfunc_selector(CatBossMonster::attackActionFinished)), NULL);
...
void CatBossMonster::isAttackEffect(){
...
void CatBossMonster::moveActionFinished() {
    
    isFinishMoveAction = true;
}
void CatBossMonster::attackActionFinished(){
    
    isFinishAttackAction = true;
}
void CatBossMonster::hittedActionFinished(){
    
    isFinishHittedAction = true;
}
void CatBossMonster::update(float dt) {
