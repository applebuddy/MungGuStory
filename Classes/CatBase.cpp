...
CatBase::CatBase(int monsterNumber)
{
...
void CatBase::loadMonsterSpriteData()
{
    ...
    now_hp = hp;
    damageValue = 50;
    attackRangeValue = 10*winSize.width/1200;
    moveVelocity = 60*winSize.width/1200;
    dieMoney = 160;
    ...
    unitSize = 2;
    //기본 이동 액션
    cache->addSpriteFramesWithFile("CatBase.plist");
    for (int i = 0; i<4; i++) {
        char *pString[20] = {};
        sprintf((char *)pString, "CatBase_%d.png", i);
        moveFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    cache->addSpriteFramesWithFile("CatBase_attack.plist");
    for (int i = 0; i<4; i++){
        char *pString[20] = {};
        sprintf((char *)pString, "CatBase_attack_%d.png", i);
        attackFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    cache->addSpriteFramesWithFile("CatBase_die.plist");
    dieFrames.pushBack(cache->getSpriteFrameByName("CatBase_die.png"));
    ...
    animation.attackFrameValue = 1.0f / 11.0f; //공격 이미지 전환 속도
    animation.waitFrameValue = 0.2f;
    ...
    char *pString[20] = {};
    sprintf((char *)pString, "CatBase_attack_%d.png", 0);
    ...
    monsterStopImage[0] = cache->getSpriteFrameByName("CatBase_0.png");
    //monsterDeadImage[0] = cache->getSpriteFrameByName("Cat_die.png");
    this->setScale(0.8*winSize.width/1200);
    cache->removeSpriteFrames();
...
void CatBase::monsterAction(int actionNumber)
{
    switch (actionNumber) {
        case ACTION_MOVE:
            moveAction();//움직이는 액션
...
void CatBase::setHpBar(){
    ...
    catEnergyTimer->setPosition(Point(mungRect.getMaxX(),mungRect.getMaxY()*2));
    this->addChild(catEnergyTimer,10);
}

void CatBase::deadAction(){//구현중..
    ...
    if (isFinishHittedAction == true){
        ...
        auto hittedMove = JumpBy::create(0.6*frameRate, Point(20*winSize.width/1200 * 1.1, 0), 35, 1);
        auto hittedMove2 = JumpBy::create(0.3*frameRate, Point(13*winSize.width/1200 * 1.1, 0), 20, 1);
        auto hittedMove3 = JumpBy::create(0.2*frameRate, Point(8*winSize.width/1200 * 1.1, 0), 10, 1);
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(CatBase::deadAndDisappear)), NULL);
...
void CatBase::angryAction(){
    auto act0 = ScaleBy::create(0, 1.2); //구현중
    auto act = Sequence::create(CallFunc::create(this, callfunc_selector(CatBase::angryEffect)), act0, NULL);
    act->setTag(ACTION_ANGRY);
    runAction(act);
    
}

void CatBase::angryEffect(){//구현중
    //this->setScale(2.0);
    this->hp = hp*1.3;
    this->now_hp = hp;
...
void CatBase::hittedAction(){
    ...
    if ((isFinishHittedAction == true)&&(now_hp!=0)){
        ...
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(CatBase::hittedActionFinished)), NULL);
...
void CatBase::stopAction() {//게임신에서 멈춤 지시가 오면
...
void CatBase::moveAction() {
    if ((isFinishMoveAction == true) && (isFinishHittedAction == true)){//움직임 액션이 끝났으면 움직임 액션 진행
        
        isFinishMoveAction = false;
        FiniteTimeAction* moveAction = Sequence::create(animation.animate_move, CallFunc::create(this, callfunc_selector(CatBase::moveActionFinished)), NULL);//
        FiniteTimeAction* moveDistance = Sequence::create(MoveTo::create(animation.moveFrameValue * 4.5, Point(getPositionX() - moveVelocity * 1, getPositionY())), NULL);
...
void CatBase::attackAction(){
    if ((isFinishAttackAction == true) && (isFinishHittedAction == true)) {
        ...
        auto act1 = Sequence::create(act00, CallFunc::create(this, callfunc_selector(CatBase::isAttackEffect)), NULL);
        FiniteTimeAction* attackAction = Sequence::create(animation.animate_attack,animation.animate_wait, CallFunc::create(this, callfunc_selector(CatBase::attackActionFinished)), NULL);
...
void CatBase::moveActionFinished() {
    
    isFinishMoveAction = true;
}
void CatBase::attackActionFinished(){
    
    isFinishAttackAction = true;
}
void CatBase::hittedActionFinished(){
    
    isFinishHittedAction = true;
}
void CatBase::update(float dt) {
