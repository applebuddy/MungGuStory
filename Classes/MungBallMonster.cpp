...
MungBallMonster::MungBallMonster(int monsterNumber)
{
...
MungBallMonster::~MungBallMonster()
{
    
    
}

void MungBallMonster::loadMonsterSpriteData()
{
    ...
    int mungBallLv = UserDefault::getInstance()->getIntegerForKey("mungBallLv");
    
    hp = (150+mungBallLv*3-3);
    now_hp = hp;
    damageValue = 50+mungBallLv*5-5;
    attackRangeValue = 13*winSize.width/1200;
    moveVelocity = 50*winSize.width/1200;
    if(mungBallLv>=10){
        moveVelocity = 56;
    }
    mySizeDecreaseRate = 10*winSize.width/1200;
    
    unitSize = 1;
    
    //기본 이동 액션
    cache->addSpriteFramesWithFile("mungBall.plist");
    for (int i = 0; i<5; i++) {
        char *pString[20] = {};
        sprintf((char *)pString, "mungBall_%d.png", i);
        moveFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    cache->addSpriteFramesWithFile("mungBall_attack.plist");
    for (int i = 0; i<6; i++){
        char *pString[20] = {};
        sprintf((char *)pString, "mungBall_attack_%d.png", i);
        attackFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    Vector<SpriteFrame *> dieFrames;
    cache->addSpriteFramesWithFile("mungBall_die.plist");
    dieFrames.pushBack(cache->getSpriteFrameByName("mungBall_die.png"));
    ...
    animation.attackFrameValue = 1.0f / 10.0f; //공격 이미지 전환 속도
    if(mungBallLv>=10){
    ...
    //animation.attackmoveFrameValue = 5 * animation.attackFrameValue;
    ...
    monsterStopImage[0] = cache->getSpriteFrameByName("mungBall_0.png");
    scaleRate = 1.06*winSize.width/1200+0.008*(float)mungBallLv-0.008;
    this->setScale(scaleRate);
    cache->removeSpriteFrames();
    
}

void MungBallMonster::monsterAction(int actionNumber)
{
...
void MungBallMonster::setHpBar(){
    ...
    mungEnergyTimer->setScale(0.8);
    mungEnergyTimer->setPosition(Point(mungRect.getMidX()*1.35,mungRect.getMaxY()*3));
    this->addChild(mungEnergyTimer,10);
    //    mungEnergyTimer->setAnchorPoint(Point(0.5,0));
    
}

void MungBallMonster::deadAction(){//구현중..
    
    if (isFinishDiedAction == false){
        isFinishDiedAction = true;
        
        mungEnergyTimer->setVisible(false);
        ...
        auto hittedMove = JumpBy::create(0.6*frameRate, Point(-35*winSize.width/1200, 0), 35*winSize.width/1200, 1);
        auto hittedMove2 = JumpBy::create(0.3*frameRate, Point(-20*winSize.width/1200, 0), 20*winSize.width/1200, 1);
        auto hittedMove3 = JumpBy::create(0.2*frameRate, Point(-10*winSize.width/1200, 0), 10*winSize.width/1200, 1);
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(MungBallMonster::deadAndDisappear)), NULL);
...
void MungBallMonster::deadAndDisappear(){
    double actionRate = rand()%2+1;
    double moveRate = rand()%55+5;
...
void MungBallMonster::hittedAction(){
    
    if ((isFinishHittedAction == true)&&(now_hp!=0)){
        isFinishHittedAction = false;
        
        
        auto hittedMove = JumpBy::create(1.3, Point(-35*winSize.width/1200, 0), 35, 1);
        auto hittedMove2 = JumpBy::create(0.6, Point(-20*winSize.width/1200, 0), 20, 1);
        auto hittedMove3 = JumpBy::create(0.3, Point(-10*winSize.width/1200, 0), 10, 1);
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(MungBallMonster::hittedActionFinished)), NULL);
...
void MungBallMonster::stopAction() {
    stopAllActions();
    isCallStopAction = true;
    isFinishMoveAction = true;
}

void MungBallMonster::moveAction() {
    if ((isFinishMoveAction == true)&&(isFinishHittedAction == true)){//움직임 액션이 끝났으면 움직임 액션 진행
        ...
        FiniteTimeAction* moveAction = Sequence::create(animation.animate_move, CallFunc::create(this, callfunc_selector(MungBallMonster::moveActionFinished)), NULL);
        FiniteTimeAction* moveDistance = Sequence::create(MoveTo::create(animation.moveFrameValue * 5, Point(getPositionX() + moveVelocity * 1, getPositionY())), NULL);
...
void MungBallMonster::attackAction(){
    if ((isFinishAttackAction == true)&&(isFinishHittedAction == true)) {
        ...
        auto act1 = Sequence::create(act00, CallFunc::create(this, callfunc_selector(MungBallMonster::isAttackEffect)), NULL);
        FiniteTimeAction* attackAction = Sequence::create(animation.animate_attack, CallFunc::create(this, callfunc_selector(MungBallMonster::attackActionFinished)), NULL);
...
void MungBallMonster::isAttackEffect(){
...
void MungBallMonster::moveActionFinished() {
    isFinishMoveAction = true;
}
void MungBallMonster::attackActionFinished(){
    isFinishAttackAction = true;
}
void MungBallMonster::hittedActionFinished(){
    isFinishHittedAction = true;
    
}
void MungBallMonster::update(float dt) {
