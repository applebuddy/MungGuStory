...
CatInuMonster::CatInuMonster(int monsterNumber)
{
...
CatInuMonster::~CatInuMonster()
{
    
    
}

void CatInuMonster::initData()
{
    winSize = Director::getInstance()->getWinSize();//현재 화면의 크기를 가져와 winSize에 정보를 담음
    isEnemyAngryed = false;
    isMultiAttack = true;
    enemySizeDecreaseRate = 110*winSize.width/1200;
}


void CatInuMonster::loadMonsterSpriteData()
{
    ...
    now_hp = hp;
    damageValue = 135;
    attackRangeValue = 36*winSize.width/1200;
    moveVelocity = 30*winSize.width/1200;
    dieMoney = 250;
    unitSize = 2;
    //기본 이동 액션
    cache->addSpriteFramesWithFile("CatInu.plist");
    for (int i = 0; i<4; i++) {
        char *pString[20] = {};
        sprintf((char *)pString, "CatInu_%d.png", i);
        moveFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    
    
    cache->addSpriteFramesWithFile("CatInu_attack.plist");
    for (int i = 0; i<7; i++){
        char *pString[20] = {};
        sprintf((char *)pString, "CatInu_attack_%d.png", i);
        attackFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    char *pString[20] = {};
    sprintf((char *)pString, "CatInu_attack_%d.png", 0);
    waitFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    
    
    cache->addSpriteFramesWithFile("CatInu_die.plist");
    dieFrames.pushBack(cache->getSpriteFrameByName("CatInu_die.png"));
    ...
    animation.attackFrameValue = 1.0f / 23.0f; //공격 이미지 전환 속도
    animation.waitFrameValue = 5.6f;
    ...
    monsterStopImage[0] = cache->getSpriteFrameByName("CatInu_attack_0.png");
    //monsterDeadImage[0] = cache->getSpriteFrameByName("Cat_die.png");
    scaleRate = 1.4*winSize.width/1200;
...
void CatInuMonster::monsterAction(int actionNumber)
{
    switch (actionNumber) {
        case ACTION_MOVE:
            moveAction();//움직이는 액션
...
void CatInuMonster::setHpBar(){
    ...
    catEnergyTimer->setAnchorPoint(Point(0.36,0));
    catEnergyTimer->setScale(1+(1-scaleRate));
    catEnergyTimer->setPosition(Point(mungRect.getMaxX(),mungRect.getMaxY()*1.6));
    this->addChild(catEnergyTimer,10);
}

void CatInuMonster::deadAction(){//구현중..
    ...
    if (isFinishHittedAction == true){
        ...
        auto hittedMove = JumpBy::create(0.6*frameRate, Point(30*winSize.width/1200 * 1.1, 0), 35, 1);
        auto hittedMove2 = JumpBy::create(0.3*frameRate, Point(24*winSize.width/1200 * 1.1, 0), 20, 1);
        auto hittedMove3 = JumpBy::create(0.2*frameRate, Point(10*winSize.width/1200 * 1.1, 0), 10, 1);
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(CatInuMonster::deadAndDisappear)), NULL);
...
void CatInuMonster::angryAction(){
    auto act0 = ScaleBy::create(0, 1.2); //구현중
    auto act = Sequence::create(CallFunc::create(this, callfunc_selector(CatInuMonster::angryEffect)), act0, NULL);
    act->setTag(ACTION_ANGRY);
    runAction(act);
    
}

void CatInuMonster::angryEffect(){//구현중
    //this->setScale(2.0);
    this->hp = hp*1.3;
    this->now_hp = hp;
...
void CatInuMonster::deadAndDisappear(){
...
void CatInuMonster::hittedAction(){
    
    if ((isFinishHittedAction == true)&&(now_hp!=0)){
        isFinishHittedAction = false;
        
        
        auto hittedMove = JumpBy::create(0.5, Point(20, 0), 35, 1);
        auto hittedMove2 = JumpBy::create(0.3, Point(15, 0), 20, 1);
        auto hittedMove3 = JumpBy::create(0.2, Point(7, 0), 10, 1);
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(CatInuMonster::hittedActionFinished)), NULL);
...
void CatInuMonster::stopAction() {//게임신에서 멈춤 지시가 오면
...
void CatInuMonster::moveAction() {
    if ((isFinishMoveAction == true) && (isFinishHittedAction == true)){//움직임 액션이 끝났으면 움직임 액션 진행
        
        isFinishMoveAction = false;
        FiniteTimeAction* moveAction = Sequence::create(animation.animate_move, CallFunc::create(this, callfunc_selector(CatInuMonster::moveActionFinished)), NULL);//
...
void CatInuMonster::attackAction(){
    if ((isFinishAttackAction == true) && (isFinishHittedAction == true)) {
        ...
        auto act0 = DelayTime::create(3);//3번째 프레임에서 공격
        auto act00 = DelayTime::create(animation.attackFrameValue * 3);//3번째 플레임에서 공격
        auto act1 = Sequence::create(act00, CallFunc::create(this, callfunc_selector(CatInuMonster::isAttackEffect)), NULL);
        FiniteTimeAction* attackAction = Sequence::create(animation.animate_attack, NULL);
        
        auto act2 = Spawn::create(act1, attackAction, NULL);
        auto act3 = Sequence::create(act2,animation.animate_wait,CallFunc::create(this, callfunc_selector(CatInuMonster::attackActionFinished)),NULL);
        runAction(act3);
...
void CatInuMonster::isAttackEffect(){
...
void CatInuMonster::moveActionFinished() {
    
    isFinishMoveAction = true;
}
void CatInuMonster::attackActionFinished(){
    
    isFinishAttackAction = true;
}
void CatInuMonster::hittedActionFinished(){
    
    isFinishHittedAction = true;
}
void CatInuMonster::update(float dt) {
