...
CatNamGu::CatNamGu(int monsterNumber)
{
...
void CatNamGu::initData()
{
    winSize = Director::getInstance()->getWinSize();//현재 화면의 크기를 가져와 winSize에 정보를 담음
    isEnemyAngryed = false;
    isMultiAttack = true;
    enemySizeDecreaseRate = 20*winSize.width/1200;
}


void CatNamGu::loadMonsterSpriteData()
{
    ...
    isStrongAboutSmall=true;//작은 상대한테 세다.
    isStrongAboutBig=false;//
    unitSize = 2;
    
    
    //기본 이동 액션
    cache->addSpriteFramesWithFile("CatNamGu.plist");
    for (int i = 0; i<4; i++) {
        char *pString[20] = {};
        sprintf((char *)pString, "CatNamGu_%d.png", i);
        moveFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    cache->addSpriteFramesWithFile("CatNamGu_attack.plist");
    for (int i = 0; i<9; i++){
        char *pString[20] = {};
        sprintf((char *)pString, "CatNamGu_attack_%d.png", i);
        attackFrames.pushBack(cache->getSpriteFrameByName((const char*)pString));
    }
    
    cache->addSpriteFramesWithFile("CatNamGu_die.plist");
    dieFrames.pushBack(cache->getSpriteFrameByName("CatNamGu_die.png"));
    ...
    animation.attackFrameValue = 1.0f / 18.0f; //공격 이미지 전환 속도
    animation.waitFrameValue = 0.05f;
    ...
    char *pString[20] = {};
    sprintf((char *)pString, "CatNamGu_attack_%d.png", 0);
    ...
    monsterStopImage[0] = cache->getSpriteFrameByName("CatNamGu_0.png");
    //monsterDeadImage[0] = cache->getSpriteFrameByName("Cat_die.png");
    scaleRate = 0.74*winSize.width/1200;
...
void CatNamGu::monsterAction(int actionNumber)
{
    switch (actionNumber) {
        case ACTION_MOVE:
            moveAction();//움직이는 액션
...
void CatNamGu::setHpBar(){
    ...
    catEnergyTimer->setPosition(Point(mungRect.getMaxX(),mungRect.getMaxY()*2.8));
    this->addChild(catEnergyTimer,10);
}

void CatNamGu::deadAction(){//구현중..
    ...
    if (isFinishHittedAction == true){
        ...
        auto hittedMove = JumpBy::create(0.8*frameRate, Point(50*winSize.width/1200 * 1.1, 0), 35, 1);
        auto hittedMove2 = JumpBy::create(0.4*frameRate, Point(30*winSize.width/1200 * 1.1, 0), 20, 1);
        auto hittedMove3 = JumpBy::create(0.3*frameRate, Point(20*winSize.width/1200 * 1.1, 0), 10, 1);
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(CatNamGu::deadAndDisappear)), NULL);
...
void CatNamGu::angryAction(){
    auto act0 = ScaleBy::create(0, 1.2); //구현중
    auto act = Sequence::create(CallFunc::create(this, callfunc_selector(CatNamGu::angryEffect)), act0, NULL);
    act->setTag(ACTION_ANGRY);
    runAction(act);
    
}

void CatNamGu::angryEffect(){//구현중
    //this->setScale(2.0);
    this->hp = hp*1.3;
    this->now_hp = hp;
...
void CatNamGu::deadAndDisappear(){
    ...
    double rotateRate = rand() % 1080 + 100;
...
void CatNamGu::hittedAction(){
    ...
    if ((isFinishHittedAction == true)&&(now_hp!=0)){
        ...
        FiniteTimeAction* pAction = Sequence::create(hittedMove, hittedMove2, hittedMove3, CallFunc::create(this, callfunc_selector(CatNamGu::hittedActionFinished)), NULL);
...
void CatNamGu::stopAction() {//게임신에서 멈춤 지시가 오면
...
void CatNamGu::moveAction() {
    if ((isFinishMoveAction == true) && (isFinishHittedAction == true)){//움직임 액션이 끝났으면 움직임 액션 진행
        isFinishMoveAction = false;
        FiniteTimeAction* moveAction = Sequence::create(animation.animate_move, CallFunc::create(this, callfunc_selector(CatNamGu::moveActionFinished)), NULL);//
...
void CatNamGu::attackAction(){
    if ((isFinishAttackAction == true) && (isFinishHittedAction == true)) {
        ...
        auto act1 = Sequence::create(act00, CallFunc::create(this, callfunc_selector(CatNamGu::isAttackEffect)), NULL);
        FiniteTimeAction* attackAction = Sequence::create(animation.animate_attack,animation.animate_wait, CallFunc::create(this, callfunc_selector(CatNamGu::attackActionFinished)), NULL);
...
void CatNamGu::moveActionFinished() {
    
    isFinishMoveAction = true;
}
void CatNamGu::attackActionFinished(){
    
    isFinishAttackAction = true;
}
void CatNamGu::hittedActionFinished(){
    
    isFinishHittedAction = true;
}
void CatNamGu::update(float dt) {
